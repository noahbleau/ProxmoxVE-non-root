...
function validate_service_script() {
  ...
  curl -fsSL --max-time 10 -o /dev/null \
    "https://raw.githubusercontent.com/noahbleau/ProxmoxVE-non-root/root-check-bypass/ct/${name}.sh" 2>/dev/null
}
...
for container in $CHOICE; do
  ...
  script=$(curl -fsSL "https://raw.githubusercontent.com/noahbleau/ProxmoxVE-non-root/root-check-bypass/ct/${service}.sh")
